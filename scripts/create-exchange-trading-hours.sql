-- Create exchange status management system
...
BEGIN
    ...
    WHERE DATE_TRUNC('week', matched_at) < DATE_TRUNC('week', NOW());
...
CREATE INDEX IF NOT EXISTS idx_buy_orders_week_status ON buy_orders(DATE_TRUNC('week', created_at), status);
CREATE INDEX IF NOT EXISTS idx_sell_orders_week_status ON sell_orders(DATE_TRUNC('week', created_at), status);
