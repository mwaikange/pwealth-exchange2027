-- Enhanced Weekly Matching System with Fixed Variable Names
...
BEGIN
    ...
    -- Expire unfilled buy orders from previous weeks
    UPDATE buy_orders SET
        status = 'expired'::order_status,
        updated_at = NOW()
    WHERE status IN ('pending', 'partial')
    AND created_at < current_week_start::TIMESTAMPTZ;
    
    GET DIAGNOSTICS expired_buy_count = ROW_COUNT;
    
    -- Expire unfilled sell orders from previous weeks
    UPDATE sell_orders SET
        status = 'expired'::order_status,
        updated_at = NOW()
    WHERE status IN ('available', 'partial')
...
BEGIN
    ...
    weekly_price_exists := (current_price IS NOT NULL);
            ...
            'next_trading_window', 
                CASE 
                    WHEN current_day = 0 OR (current_day = 1 AND current_hour < 9) THEN
                        current_week_start::TIMESTAMPTZ + INTERVAL '1 day 9 hours 10 minutes'
                    ELSE current_time
                END
        );
    ELSE
...
-- 9. Initialize current week's price if not exists
DO $$
DECLARE
    current_week_start DATE;
...
SELECT 'Testing is_trading_allowed()' as test;
SELECT is_trading_allowed();

SELECT 'Testing set_weekly_price()' as test;
SELECT set_weekly_price();

SELECT 'Testing match_orders()' as test;
SELECT match_orders();

SELECT 'System setup complete!' as status;
